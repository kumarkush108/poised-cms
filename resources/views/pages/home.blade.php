...
@section('content')
    ...
    @php
        ...
        // Content is always left / visual always right in the v2 split-panel hero — no
        // more per-slide side alternation, which existed only to vary a full-bleed
        // photo layout that no longer exists.
        $slideConfig = [
            0 => ['image' => 'carousel-3.png', 'alt' => 'EV innovation'],
            1 => ['image' => 'carousel-1.png', 'alt' => 'Digital transformation'],
            2 => ['image' => 'carousel-2.png', 'alt' => 'Scalable technology'],
        ];
                ...
                @foreach ($heroSlides as $index => $slide)
                    ...
                    <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                        <div class="hero-slide">
                            <div class="container">
                                <div class="hero-slide__grid">

                                    <div class="hero-slide__content hero-caption">
                                        <span class="hero-kicker">Poised Technology</span>
                                        ...
                                        @if ($subheading = Content::itemField($slide, 'subheading'))
                                            <p>{!! Content::richtext($subheading) !!}</p>
                                        @endif

                                        @if ($body = Content::itemField($slide, 'body'))
                                            <p>{!! Content::richtext($body) !!}</p>
                                        @endif

                                        @if ($buttonText = Content::itemField($slide, 'button_text'))
                                            <div class="hero-actions">
                                                <a href="{{ Content::itemField($slide, 'button_url', '') }}" class="btn-cta-lg">{{ $buttonText }}</a>
                                                <a href="{{ route('contact') }}" class="btn-ghost-lg">Get in Touch</a>
                                            </div>
                                        @endif

                                        @if (count($heroSlides) > 1)
                                            <div class="carousel-indicators hero-indicators">
                                                @foreach ($heroSlides as $dotIndex => $dotSlide)
                                                    <button type="button" data-bs-target="#header-carousel" data-bs-slide-to="{{ $dotIndex }}"
                                    ...
                                    <div class="hero-slide__visual">
                                        <div class="hero-slide__blob"></div>
                                        <div class="hero-slide__frame">
                                            <img src="{{ $bgImage }}" alt="{{ $cfg['alt'] }}">
                                        </div>
                                    </div>
